...
# --- Blade beam properties (GFRP hollow aerofoil section) ---
BEAM_E              = 2.07e5     # N/mm^2  Young's modulus (steel-equiv for stiffness)
BEAM_G              = 8.0e4     # N/mm^2  Shear modulus
BEAM_AREA           = 15_000.0   # mm^2    Cross-section area
BEAM_IXX            =  4.0e9     # mm^4   Torsional second moment
BEAM_IYY            =  2.0e9     # mm^4   Flapwise bending  (EI = 4.1e14 N.mm^2)
BEAM_IZZ            =  8.0e9     # mm^4   Edgewise bending
...
# Rotor spin: velocity ramps from 0 to ROTOR_DEG_PER_SEC over 1 s using STEP,
# then holds constant.  This avoids the t=0 velocity discontinuity that causes
# Adams to fail computing initial accelerations when stiff beams are present.
ROTOR_RAMP_TIME = 1.0  # s
m.Constraints.createJointMotion(
    name='hub_spin',
    joint=rev_hub,
    type_of_freedom='rotational',
    time_derivative='velocity',
    function=f'STEP(TIME, 0, 0, {ROTOR_RAMP_TIME}, {ROTOR_DEG_PER_SEC}D)',
)
    ...
    for i in range(N_BLADE_SEGMENTS):
        ...
        if i == 0:
            ...
            create_beam(
                ...
                i_marker=beam_mkr,       # outboard end (I must be outboard: J→I is +X)
                j_marker=prev_beam_mkr,  # inboard end
                length=SEG_LEN,
                E=BEAM_E, G=BEAM_G,
                area=BEAM_AREA,
