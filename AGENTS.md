...
- **Launching Adams View (`open_view_here.ts.js`, `open_in_view.ts.js`) — use a hidden integrated terminal, NOT `child_process`.** These invoke MSC's `mdi.bat`, whose launcher chain (`mdi.bat` → `menu.exe` → `run_mdi.py` → `os.system("call adamsctl_<RAND>.bat")`) **requires a real console** to complete; without one it fails (`adamsctl… not recognized`) and still exits 0, so the launch fails silently. The extension host is a GUI process with no console, so every `child_process` approach is console-less from its perspective: `execFile` couldn't launch the `.bat`; `spawn({detached})`/`wscript` flashed console windows and/or silently failed; and `exec` (`cmd /d /s /c`, piped I/O) silently fails too (it only *looked* fixed in the Extension Development Host, which inherits a different context). The working approach is `vscode.window.createTerminal({ shellPath: cmd.exe, cwd, hideFromUser: true })` + `terminal.sendText(...)`: the integrated terminal runs cmd in a ConPTY (a real pseudo-console), with no flashing window. Do not "fix" this back to `exec`/`execFile`/`spawn` — see CHANGELOG 2.2.4. (General rule for *other* external-process launches: prefer `execFile(file, args)` over `exec()` to avoid shell injection.)
