...
|---|---|---|
| `spring_damper_name` | An Existing Spring-damper Force | Specifies the spring-damper force to modify. You use this parameter to identify the existing spring-damper to affect with this command. You may identify a spring-damper force by typing its name or by picking it from the screen. |
...
| `preload` | Force | Specifies the reference force or torque for the spring. This is the force the spring exerts when the displacement between the I and J markers is equal to DISPLACEMENT_AT_PRELOAD (the reference length of the spring). |
